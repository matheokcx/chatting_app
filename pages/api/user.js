import mysql from 'mysql2/promise'; // Utiliser mysql2 pour les promesses
    ...
    if (req.method === 'GET') {
        ...
        try {
            const [valeurs] = await bdd.query(`
                ...
                FROM user u JOIN conversation c ON u.idUser = c.userid2
                WHERE c.userid1 = (SELECT idUser FROM user WHERE mail = ?)
                UNION
                SELECT DISTINCT u.mail 
                FROM user u JOIN conversation c ON u.idUser = c.userid1
                WHERE c.userid2 = (SELECT idUser FROM user WHERE mail = ?);
            `, [userMail, userMail]);
        ...
        try {
            ...
            const [testConversationExiste] = await bdd.query(`
                SELECT idConversation FROM conversation 
                WHERE (userid1 = ? AND userid2 = ?) OR (userid1 = ? AND userid2 = ?);
            `, [monIdentifiant[0].idUser, autreIdentifiant[0].idUser, autreIdentifiant[0].idUser, monIdentifiant[0].idUser]);
            ...
            if (autreIdentifiant.length > 0 && testConversationExiste.length === 0) {
                ...
                res.status(400).send('La conversation existe déjà ou l\'autre utilisateur n\'existe pas');
            }
        } catch (error) {
            res.status(500).send(error.message);
        }
